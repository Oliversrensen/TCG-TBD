...
- **No database in current scope.** Matchmaking and game state are in-memory.
...
| **server/game/** | Pure game logic (state, actions, cards). No I/O. | **Rules:** `state.ts`. **Card definitions:** `cards.ts`. **Types:** `types.ts`. |
...
- **Add a new card:** Edit `server/game/cards.ts` (CARD_TEMPLATES, and deck list if using deck). Update types in `types.ts` only if the card has new fields (e.g. new keyword).
...
- **Single source of truth:** Game state lives only on the server. Clients never compute next state; they only send intents and render received state.
